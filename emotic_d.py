...
class Emotic_DataLoader(Dataset):
    def __init__(self, config, mode, emo_mode):
        ...
        #emo_dir = 'emotic_cat_oneshot_2'
        if mode == 'Samples':
           emo_dir = 'emotic_sample_'+str(emo_mode)
           #emo_dir = 'emotic_cat_samples_2'
        ...
        #if mode == 'Train':
        #   cato_list = ['A01','A02' ,'A03','A04','A05', 'A06']
        #elif mode == 'Test':
        ...
        print(mode, len(self.file_list_context))
    ...
    def __getitem__(self,index):
        ...
        name = int(context_path.split('/')[-1].split('.')[0])
        cat_name = np.ones([3,1,256], dtype=np.float32)*name
        data = np.concatenate([context,body,semantic, cat_name],axis=-2)
        #print(data.shape)
        return data,label
