...
def init_weights(m):
    if type(m) == nn.Linear:
        torch.nn.init.normal(m.weight, 0.4, 0.002)
...
class trunk_2(nn.Module):
    def __init__(self,cfg):
        ...
        self.semantic = nn.Sequential(nn.Linear(150,256), nn.ReLU(),nn.BatchNorm1d(256), nn.Linear(256, 512))
    ...
    def forward(self,data):
        ...
        #print('test')
        feature_context = self.fc1(self.relu(self.trunk_context(context)))
        feature_body = self.fc2(self.relu(self.trunk_body(body)))
...
def train_app(cfg):
    ...
    experiment_name = "resnet50_emolevel_73_dominance_re_stepsize_10_%s_model_%s_cl_%s_ml_%s_miner_%s_mix_ml_%02.2f_mix_cl_%02.2f_resize_%d_emb_size_%d_class_size_%d_opt_%s_lr_%02.2f_m_%02.2f_wd_%02.2f"%(cfg.dataset.name,
